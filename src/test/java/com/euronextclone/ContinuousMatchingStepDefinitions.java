...
public class ContinuousMatchingStepDefinitions {
    ...
    @Given("^the Limit sell order from broker \"([^\"]*)\" for (\\d+) shares$")
    public void the_Limit_sell_order_from_broker_for_shares(String broker, int quantity) throws Throwable {
        matchingUnit.newOrder(Order.OrderSide.Sell, broker, quantity, new OrderPrice(MarketOrder.INSTANCE));
    }
